...
//Give the image a unique name. DOnt forget the extension
$filename = date("d_m_Y_H_i_s") . "-" . time() . $ext;

//The path to the newly ceated file inside the upload folder
$destinationPath = "$destinationFolder$filename";

//Ceate the file or return false
$success = file_put_contents($destinationPath, $img);

if($success) {
    $db = new Database();
    $db->insertUserPic($user, $destinationPath);
}
