...
const imagePickerArea = document.querySelector('#pickImage');
const newImages = document.querySelector('#newImages');

//th dimensions of the Image
const width = 320;
const height = 240;
let zIndex = 1;
...
captureButton.addEventListener('click', (event) => {
    ...
    // Convert data so it can be saved as a file
    let picture = canvasElement.toDataURL();

    //Save the file by posting it to the server
    fetch('api/save_image.php', {
        method: 'post',
        body: JSON.stringify({data: picture, user: username})
    })
    .then((res) => res.json())
    .then((data) => {
        if(data.success) {
            // Create the image and give it the CSS style with a randomized tilt
